...
function mainjs() {
  ...
  for (let thatIndex = 0; thatIndex < N; thatIndex++) {
    ...
    //console.log(availableRow)
    console.log('all the missing numbers: ' + availableNumber(availableRow, availableColumn, availableBox) + '\n')
...
function checkBox(sud, index) {
  ...
  let j = 0
  for (let i = tmp * 3 - division * L + divisionOfDivision * L*3; i < tmp * 3 - division * L + divisionOfDivision * 27 + 21; i++) {
    thisBox[j] = sud[i]
    j++
    if (i % 3 === 2) {
      i = i + 6
...
function solve(backflag, solvedSudoku, index) {
  if (backflag == undefined)
    throw 'error from can: backflag is undefined'
